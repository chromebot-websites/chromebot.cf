  ...
  componentDidMount() {
    this.state.xmlhttp.onreadystatechange = () => {
      if (this.state.xmlhttp.readyState == 4 &&
        ...
        JSON.parse(this.state.xmlhttp.responseText).members.forEach(member => {
          if (member.id == this.props.botId) {
            ...
            if (member.status === "idle") {
              this.setState(
                  {color : "#AF7E00", message : "experiencing a Minor Outage"});
            } else if (member.status === "dnd") {
              this.setState(
                  {color : "#c65b29", message : "experiencing a Major Outage"});
            } else {
        ...
        console.log("sending...");
        this.timeoutId = SendXMLHTTP("https://discordapp.com/api/guilds/" +
                                         this.props.serverId + "/widget.json",
                                     this.state.xmlhttp, 5000);
      }
    };
  }
