	...
	renderSpecials(member) {
		if (member.specials) {
			return member.specials.sort((special1, special2) => {
				...
				return (
					<span className="disable-select tooltip" data-tooltip={specials[special].description} style={{fontSize: "80%"}}>{specials[special].emoji}</span>
				);
			});
		}
	}
...
class Tag extends Component {
	...
	render() {
		return (
			<b style={{color: this.state.color, backgroundColor: this.state["background-color"], borderRadius: "10px"}}>[{this.state.title}]</b>
		);
	}
}
